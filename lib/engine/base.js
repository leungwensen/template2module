var declare = require('zero-declare');
...
var Engine = declare({
    ...
    parse: function (str/*, filePath, config*/) {
        ...
        var me = this;
        var functionBody = 'var _s = "' + str.replace(/"/g, '\"') + '";';
        return lang.extend({
    ...
    generateArguments: function (functionBody) {
        ...
        function getContextVars(context) {
            ...
            var vardeclMap = context.vardeclMap;
            var params = context.getParams();

            lang.each(vars, function (v) {
                if (lang.contains(params, v) || lang.hasKey(vardeclMap, v)) hash[v] = true;
            });
            lang.each(vids, function (v) {
                if (lang.contains(params, v) || hash[v]) return;
                hash[v] = true;
                args.push(v);
            });
        ...
        var context = homunculus.getContext('js')
            .parse('function template(){ ' + functionBody + ' }')
            .getChildren()[0];
